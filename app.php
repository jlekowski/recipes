...
$app->post('/recipes/:id/ingredients', function() use ($app, $oDB) {
    ...
    $app->response->setStatus(201);
    $app->response->headers->set('Location', '/recipes/' . json_decode($app->request->getBody())->recipe_id . '/ingredients/' . $id);
});

$app->put('/recipes/:id/ingredients/:recipeIngredientId', function() use ($app, $oDB) {
    $oRecipeIngredient = new RecipeIngredient($oDB);
...
$app->get('/recipes/:id/ingredients/:recipeIngredientId', function($id, $recipeIngredientId) use ($app, $oDB) {
    $oRecipeIngredient = new RecipeIngredient($oDB);
    $recipeIngredient = $oRecipeIngredient->get($recipeIngredientId);
...
$app->delete('/recipes/:id/ingredients/:recipeIngredientId', function($id, $recipeIngredientId) use ($app, $oDB) {
    $oRecipeIngredient = new RecipeIngredient($oDB);
    $oRecipeIngredient->delete($recipeIngredientId);
...
$app->post('/ingredients', function() use ($app, $oDB) {
    ...
    $app->response->setStatus(201);
    $app->response->headers->set('Location', '/ingredients/' . $id);
});
