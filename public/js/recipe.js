...
$().ready(function() {
    $('#recipe-weight').on('change', function() {
        var $this = $(this);
        $this.closest('tr').find('.per100-weight').each(function() {
            var value = Math.round($(this).siblings('.total-weight').text() / $this.val() * 10000) / 100;
            $(this).text(value);
        });
    });
